...
export default function EditSeats() {
        ...
        .patch(
          "http://localhost:8082/api/reservations/cancelhalfreservation/",
          {
            data: {
              ...
              departureTicket: Ticket.departureTicket,
            },
          }
        )
        .then(() => {
  ...
  const handleToPayment = async () => {
    ...
    axios
      .post("http://localhost:8082/api/reservations/payment", {
        totalPrice: totalPrice,
      })
      .then((res) => {
        setOpenFailure(false);
  ...
  const checkPrev = () => {
    return (
      JSON.stringify(
        economySelected.sort((a, b) => a.seatIndex < b.seatIndex)
      ) ===
        JSON.stringify(prevEconomy.sort((a, b) => a.seatIndex < b.seatIndex)) &&
      JSON.stringify(
        firstSelected.sort((a, b) => a.seatIndex < b.seatIndex)
      ) ===
        JSON.stringify(prevFirst.sort((a, b) => a.seatIndex < b.seatIndex)) &&
      JSON.stringify(
        businessSelected.sort((a, b) => a.seatIndex < b.seatIndex)
      ) ===
        JSON.stringify(prevBusiness.sort((a, b) => a.seatIndex < b.seatIndex))
    );
  };
  ...
  return (
    <Grid style={styles.background} container>
      <Grid>
        ...
        >
          <Alert severity="error" variant="filled" action={failureButtons}>
        ...
        >
          ...
          >
            <TicketDetails
              ...
              economySelected={economySelected}
              totalPrice={totalPrice}
            />
          </DialogContent>
          <DialogActions>
        ...
        <Grid class="plane-container">
          ...
          <Grid class="container">
            ...
            >
              ...
              {flight?.firstPrice &&
                splitArray([].concat(flight.firstSeats)).map(
                  (row, rowNumber) => (
                    <Grid key={rowNumber} class="row">
                      {row.map(
                        (element, seatNumber) => (
                          console.log(flight.firstPrice),
                          (
                            <Seat
                              id={id}
                              ...
                              available={
                                seatsData.firstSeatsAdults.includes(
                                  seatNumber + 8 * rowNumber
                                )
                                  ? false
                                  : seatsData.firstSeatsChildren.includes(
                                      seatNumber + 8 * rowNumber
                                    )
                                  ? false
                                  : element
                    ...
                    <Grid key={rowNumber} class="row">
                      {row.map((element, seatNumber) => (
                        <Seat
                          ...
                          setTotalPrice={setTotalPrice}
                          available={
                            seatsData.businessSeatsAdults.includes(
                              seatNumber + 8 * rowNumber
                            )
                              ? false
                              : seatsData.businessSeatsChildren.includes(
                                  seatNumber + 8 * rowNumber
                                )
                              ? false
                              : element
                    ...
                    <Grid key={rowNumber} class="row">
                      {row.map((element, seatNumber) => (
                        <Seat
                          ...
                          setTotalPrice={setTotalPrice}
                          available={
                            seatsData.economySeatsAdults.includes(
                              seatNumber + 8 * rowNumber
                            )
                              ? false
                              : seatsData.economySeatsChildren.includes(
                                  seatNumber + 8 * rowNumber
                                )
                              ? false
                              : element
                          ...
                          seatNumber={
                            ...
                            (1 +
                              rowNumber +
                              splitArray([].concat(flight.businessSeats))
                                .length +
                              splitArray([].concat(flight.firstSeats)).length)
                          }
